...
vector< vector< Point3f > > object_points; //Vektor av vektorer med 3D verdenspunkter
vector< vector< Point2f > > imagePoints1, imagePoints2; //Vektor av vektorer med 2D bildepunkter
vector< Point2f > corners1, corners2; //vektor med hjørnepunkter
vector< vector< Point2f > > left_img_points, right_img_points; //vektor av vektorer med bildepunkter
...
void load_image_points(int board_width, int board_height, int num_imgs, float square_size,
    ...
    //for løkke som går gjennom alle bildene
    for (int i = 1; i <= num_imgs; i++) {
        ...
        //Last inn bilder og omgjør til grayscale
        img1 = imread(left_img, CV_LOAD_IMAGE_COLOR);
        img2 = imread(right_img, CV_LOAD_IMAGE_COLOR);
        cvtColor(img1, gray1, CV_BGR2GRAY);
        cvtColor(img2, gray2, CV_BGR2GRAY);

        //initialiser variabler som brukes til å verifisere om hjørner blir funnet
        bool found1 = false, found2 = false;


        //Sett variabler til true dersom hjørner blir funnet
        found1 = cv::findChessboardCorners(img1, board_size, corners1,
                                           CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_FILTER_QUADS);
        found2 = cv::findChessboardCorners(img2, board_size, corners2,
                                           CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_FILTER_QUADS);

        //Hvis hjørner ikke blir funnet -> error
        if(!found1 || !found2){
            cout << "Chessboard find error!" << endl;
            cout << "leftImg: " << left_img << " and rightImg: " << right_img <<endl;
            continue;
        }
        //Dersom hjørner blir funnet rafineres posisjonen til hjørnet til subpixel nøyaktighet
        if (found1)
        {
        ...
        //vektor med alle verdenspunktene. Disse er konstante for alle bildene og har shape (n,3)
        //n er da antallet hjørner i i sjakkbrett / apriltags
        vector< Point3f > obj;
        for (int i = 0; i < board_height; i++)
            for (int j = 0; j < board_width; j++)
                obj.push_back(Point3f((float)j * square_size, (float)i * square_size, 0));
        //Dersom hjørner blir funnet i begge bilder legges de til image points og verdenspunktene legges til i objectpoints.
        //objectpoints er nå en vektor av vektorer med shape (m,(n,3)) (?)
        /*
         * object_points = [[[1,2,3],[4,5,6],[1,2,3][4,5,6],[7,8,9],[1,2,3]],[[1,2,3],[4,5,6],[1,2,3][4,5,6],[7,8,9],[1,2,3]]]
         * */
        if (found1 && found2) {
